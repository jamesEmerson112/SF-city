...
from civic_center.checkpoint import (
    capture_checkpoint, load_checkpoint, prepare_checkpoint_capture,
    save_checkpoint, write_captured_checkpoint,
)
...
def rewrite(path, mutate, *, state_hash=False, scenario_hashes=False):
    ...
    payload["sha256"] = checksum({key: value for key, value in payload.items() if key != "sha256"})
...
def test_cohort_at_work_keeps_all_ids_occupancy_and_return_events(tmp_path):
    ...
    assert sum(building["occupancy"] for building in restored.snapshot()["buildings"]) == 200
    for model in (simulation, restored):
        model.advance_ticks(10 * 3600 * model.tick_hz)
    assert restored.snapshot() == simulation.snapshot()
    assert restored.snapshot()["event_count"] == 800
    assert all(person["activity"] == "home" for person in restored.snapshot()["residents"])
...
def test_saved_document_contains_version_mode_full_scenario_compact_state_and_proofs(saved):
...
@pytest.mark.parametrize("field,value,message", [
    ("checkpoint_version", 3, "checkpoint_version"),
...
def test_scenario_checksum_catches_corrupted_route(saved):
    _simulation, path = saved
    rewrite(path, lambda payload: payload["scenario"]["nodes"][0]["position"].__setitem__(0, -100))
...
def test_state_checksum_catches_corrupted_position(saved):
    _simulation, path = saved
    rewrite(path, lambda payload: payload["state"]["residents"][0]["position"].__setitem__(0, 999))
...
@pytest.mark.parametrize("mutation", [
    ...
    lambda p: p["state"]["residents"][0]["trip"].update(segment_progress=.999),
...
def test_boolean_occupancy_cannot_compare_equal_to_integer_occupancy(saved):
    _simulation, path = saved
    rewrite(path, lambda p: p["state"]["buildings"][0].update(occupancy=False), state_hash=True)
    with pytest.raises(ValueError, match="deterministic reconstruction"):
        load_checkpoint(path)
...
def test_resigned_invalid_scenario_is_rejected_cleanly(saved):
    _simulation, path = saved
    rewrite(path, lambda p: p["scenario"]["edges"][0].update(to=["not-a-node"]), scenario_hashes=True)
...
def test_failed_atomic_replace_preserves_save_and_cleans_own_temporary(saved, monkeypatch):
    simulation, path = saved
    original = path.read_bytes()
...
def hill_scenario():
    return {
        "schema_version": 1, "id": "hill-day", "tick_hz": 10,
        "start_time_seconds": 28790,
        "nodes": [{"id": "a", "position": [0, 0, 0]}, {"id": "b", "position": [6, 0, 0]}],
        "edges": [{"id": "hill", "from": "a", "to": "b", "bidirectional": True,
                   "points": [[0, 0, 0], [3, 0, 4], [6, 0, 0]]}],
        "buildings": [{"id": "home", "entrance_node_id": "a"},
                      {"id": "work", "entrance_node_id": "b"}],
        "residents": [{"id": "walker", "home_id": "home", "work_id": "work",
                       "departure_tick": 10, "return_tick": 200, "speed_mps": 1.0}],
    }


def test_polyline_hill_checkpoint_preserves_geometry_grade_and_batched_continuation(tmp_path):
...
@pytest.mark.parametrize("field,limit,value,message", [
...
def test_capture_stays_at_exact_tick_after_live_advance_reset_and_scenario_edit(tmp_path):
    model = CivicSimulation(hill_scenario())
...
def test_preparation_is_explicit_reusable_and_bound_to_model(tmp_path):
    ...
    restored = load_checkpoint(write_captured_checkpoint(second, tmp_path / "second.json"))
    assert restored.snapshot() == model.snapshot()


@pytest.mark.parametrize("change", [
    {"tick": -1}, {"tick": True}, {"paused": 1}, {"speed": 0},
    {"state_json": b"{}"}, {"scenario_json": b"{"},
...
def test_later_daily_checkpoint_and_capture_continue_exactly_across_midnight(tmp_path, cycle):
    scenario = hill_scenario()
    ...
    captured = load_checkpoint(write_captured_checkpoint(capture_checkpoint(model), tmp_path / "captured-later.json"))
...
@pytest.mark.parametrize("field,value", [("day_index", True), ("day_seconds", -1), ("day_seconds", 86_400), ("day_index", 900)])
...
def test_checkpoint_capture_after_private_worker_model_transfer(tmp_path):
    ...
    restored = load_checkpoint(write_captured_checkpoint(capture, tmp_path / "transferred.json"))
...
def write_legacy_checkpoint(model, path):
    ...
    document = {
        "checkpoint_version": 1, "reconstruction_mode": "deterministic-scenario-v1",
        "scenario": model.scenario, "scenario_sha256": scenario_hash(model.scenario),
        "tick": model.tick, "paused": model.paused, "speed": model.speed,
        "state": state, "state_sha256": checksum(state),
    }
...
@pytest.mark.parametrize("mutation,message", [
    ...
    (lambda d: d.update(uncompressed_bytes=checkpoint.MAX_SCENARIO_BYTES + 1), "expanded scenario exceeds"),
    (lambda d: d.update(uncompressed_bytes=d["uncompressed_bytes"] - 1), "size does not match"),
    (lambda d: d.update(uncompressed_bytes=d["uncompressed_bytes"] + 1), "size does not match"),
    ...
    (lambda d: replace_compressed_bytes(d, lambda b: b[:-4]), "truncated|size does not match"),
    (lambda d: replace_compressed_bytes(d, lambda b: b + b"trailing"), "trailing or concatenated"),
    (lambda d: replace_compressed_bytes(d, lambda b: b + zlib.compress(b"{}")), "trailing or concatenated"),
])
def test_compressed_scenario_corruption_and_resource_declarations_are_rejected(saved, mutation, message):
    _model, path = saved
...
def test_expanded_tree_value_limit_cannot_be_bypassed_with_compression(tmp_path, monkeypatch):
    scenario = hill_scenario()
...
def test_expanded_scenario_and_state_share_bounded_total_bytes(saved, monkeypatch):
    _model, path = saved
    payload = json.loads(path.read_text())
    monkeypatch.setattr(checkpoint, "MAX_SCENARIO_BYTES", payload["scenario"]["uncompressed_bytes"])
...
def test_expanded_json_retains_duplicate_nonfinite_and_unicode_checks(saved, encoded):
    ...
    def change_source(payload):
        payload["scenario"].update(
            uncompressed_bytes=len(encoded), json_sha256=hashlib.sha256(encoded).hexdigest(),
            chunks=[base64.b64encode(zlib.compress(encoded)).decode("ascii")],
        )
...
def test_static_metadata_is_verified_without_repeating_it_in_dynamic_state(saved):
    _model, path = saved
    rewrite(path, lambda p: p["scenario"]["residents"][0].update(label="changed source label"), scenario_hashes=True)
...
def test_equal_length_alternate_future_geometry_cannot_pass_same_current_pose(tmp_path):
    ...
    assert alternate.snapshot(include_static=False, include_trip_geometry=False) == before
    rewrite(path, lambda p: p["scenario"]["edges"][0]["points"][2].__setitem__(1, -1), scenario_hashes=True)
    with pytest.raises(ValueError, match="metadata or trip geometry"):
        load_checkpoint(path)
...
def test_large_encoded_scenario_is_split_without_relaxing_string_limit(tmp_path):
    scenario = hill_scenario()
    scenario["opaque_source_values"] = [hashlib.sha256(str(i).encode()).hexdigest() for i in range(5000)]
    model = CivicSimulation(scenario)
...
def test_shared_dense_routes_are_stored_once_with_per_trip_verification(tmp_path):
    ...
    scenario["residents"] = [dict(resident, id=f"walker-{i}", return_tick=10_000) for i in range(20)]
    model = CivicSimulation(scenario)
    model.step_ticks(35)
    ...
    assert all("points" not in person["trip"] for person in payload["state"]["residents"])
    assert path.stat().st_size < legacy_path.stat().st_size / 4
    assert load_checkpoint(path).snapshot() == load_checkpoint(legacy_path).snapshot() == model.snapshot()
