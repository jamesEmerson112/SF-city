    ...
    def __init__(self, record: dict[str, Any]) -> None:
        ...
        for name, value in self._values.items():
            if type(value) in _JSON_SCALAR_TYPES:
                continue
            if type(value) is list and all(type(item) in _JSON_SCALAR_TYPES for item in value):
                value = value.copy()
                copier = value.copy
            else:
...
class CivicSimulation:
    ...
    def __init__(self, scenario: dict[str, Any], *, routing_backend: str | None = None) -> None:
        ...
        if "schedule" in scenario:
            ...
            if type(schedule) is not dict or set(schedule) != {"mode"} or schedule["mode"] != DAILY_SCHEDULE_MODE:
            ...
            if self._repeat_period > MAX_REPEAT_TICK // 3:
                raise ValueError("daily schedule period exceeds the JSON-safe tick range")
        ...
        self.graph = WalkingGraph(scenario.get("nodes", []), scenario.get("edges", []), backend=routing_backend)
        ...
        for source in self.scenario.get("residents", []):
            ...
            for assignment in ("home_id", "work_id"):
                if source.get(assignment) not in self._buildings:
                    raise ValueError(f"resident {resident_id} has an unknown {assignment}")
    ...
    def _validate_daily_cycles(self) -> None:
        ...
        for resident_id, source in self._sources.items():
            ...
            if departure >= self._repeat_period or return_tick > deadline:
                raise ValueError(f"resident {resident_id} daily schedule exceeds its next departure")
            ...
            if outbound is None:
                continue
            outbound_arrival = departure + self._travel_ticks(outbound, source, self.tick_hz)
            return_departure = max(outbound_arrival, return_tick)
            if return_departure > deadline:
                raise ValueError(f"resident {resident_id} outbound trip overlaps its next daily departure")
            inbound = self.graph.route(work, home)
            if inbound is None:
                continue
            home_arrival = return_departure + self._travel_ticks(inbound, source, self.tick_hz)
            if home_arrival > deadline:
                raise ValueError(f"resident {resident_id} round trip overlaps its next daily departure")
    ...
    def _advance_to(self, target_tick: int) -> None:
        if self._repeat_period:
            if target_tick > MAX_REPEAT_TICK - 2 * self._repeat_period:
                raise ValueError("repeating schedule target exceeds the JSON-safe tick range")
    ...
    def _skip_daily_periods(self, count: int, events_per_period: int) -> None:
        ...
        self.tick += shift
        self._queue = [(tick + shift, priority, identity, kind)
    ...
    def _depart(self, resident_id: str, resident: Resident, outbound: bool) -> None:
        ...
        if outbound and self._repeat_period:
            resident.cycle_index = (self.tick - source["departure_tick"]) // self._repeat_period
        ...
        if route is None:
            ...
            self._event(
                "blocked", resident_id, trip_id=trip_id, building_id=origin,
                destination_id=destination, reason=resident.blocked_reason,
            )
            return
        travel_ticks = self._travel_ticks(route, source, self.tick_hz)
        resident.trip = Trip(
            trip_id, route, self.tick, self.tick + travel_ticks,
        ...
        self._event(
            "departure", resident_id, trip_id=trip_id, building_id=origin,
    ...
    def _arrive(self, resident_id: str, resident: Resident) -> None:
        ...
        self._event(
            "arrival", resident_id, trip_id=trip.id, building_id=trip.destination_id,
        )
        if trip.direction == "outbound":
            self._schedule(
                max(self.tick, resident.source["return_tick"] + resident.cycle_index * self._repeat_period),
                resident_id, "depart_return",
            )
        elif self._repeat_period:
            self._schedule(
                resident.source["departure_tick"] + (resident.cycle_index + 1) * self._repeat_period,
                resident_id, "depart_outbound",
            )
    ...
    def trip_geometries(self, trip_ids: Iterable[str]) -> list[dict[str, Any]]:
        ...
        """
        return [dict(record, points=[list(point) for point in record["points"]],
    ...
    def capture_trip_geometries(self, trip_ids: Iterable[str]) -> list[dict[str, Any]]:
        ...
        unknown = [identity for identity in requested if identity not in self._active_trips]
        if unknown:
    ...
    def snapshot(
        self, *, include_static: bool = True, include_trip_geometry: bool = True,
    ) -> dict[str, Any]:
        ...
        for resident_id, resident in self._residents.items():
            record = self._resident_templates[resident_id].copy() if include_static else {"id": resident_id}
            trip = resident.trip
            ...
            if trip is None:
                ...
                distance = (
                    ...
                    / self.tick_hz * resident.source["speed_mps"]
                )
