"""Atomic, validated checkpoints for deterministic resident schedules.

Version 2 stores the complete scenario as bounded compressed JSON, dynamic state
once, and digests of the full metadata state and every active trip geometry.
Loading reconstructs and verifies all three; immutable routes need not be copied
into every resident record. Version 1 full-snapshot saves remain readable. Neither
format serializes Python objects, private heaps or executable code.

V2's ``scenario`` envelope declares ``zlib-json-v1``, its exact expanded byte
count, a SHA-256 of those JSON bytes, and base64 chunks of at most 64 KiB each.
The physical checkpoint and expanded scenario plus dynamic state each have a
separate 64 MiB limit; the expanded tree also shares the five-million-value
budget. Compression does not raise any nesting, container or string limit.

The ``deterministic-scenario-v1`` reconstruction mode supports the current fixed
schedules. Future edits to assignments, arbitrary commands that mutate the day,
or runtime RNG state require a new checkpoint mode with state or command-log
restoration. A state that cannot be reconstructed is rejected, never approximated.

Older scenarios without edge polylines retain their endpoint-grade geometry on
load. Checkpoints never regenerate geography or silently change saved routes.
"""
...
_DOCUMENT_FIELDS = {
    "checkpoint_version", "reconstruction_mode", "scenario", "scenario_sha256",
    "tick", "paused", "speed", "state", "state_sha256", "sha256",
}
_STATE_FIELDS = {
    "tick", "simulation_time", "clock_seconds", "paused", "speed",
    "residents", "buildings", "events", "event_count",
}
...
class PreparedCheckpointCapture:
    ...
    scenario metadata afterward is unsupported by this reconstruction mode;
    construct a new model and preparation when those inputs change. The weak
    ...
    while pending:
        ...
        if type(current) is int:
            ...
            try:
                ...
                raise ValueError("Checkpoint strings must contain valid Unicode") from error
            ...
            raise ValueError(f"Checkpoint contains a non-JSON value: {type(current).__name__}")


def _nonnegative_integer(value: Any, description: str) -> int:
    if type(value) is not int or not 0 <= value <= MAX_SAFE_INTEGER:
        raise ValueError(f"Checkpoint {description} must be a nonnegative JSON-safe integer")
    return value
...
def _encode_scenario(scenario: dict[str, Any]) -> dict[str, Any]:
    ...
    if len(encoded) > MAX_SCENARIO_BYTES:
        raise ValueError(f"Checkpoint expanded scenario exceeds {MAX_SCENARIO_BYTES} bytes")
    ...
    return {
        ...
        "chunks": [
            base64.b64encode(compressed[offset:offset + _COMPRESSED_CHUNK_BYTES]).decode("ascii")
...
def _decode_scenario(value: Any) -> dict[str, Any]:
    if type(value) is not dict or set(value) != _SCENARIO_FIELDS:
        raise ValueError("Checkpoint compressed scenario has missing or unsupported fields")
    ...
    if not 0 < size <= MAX_SCENARIO_BYTES:
        raise ValueError(f"Checkpoint expanded scenario exceeds {MAX_SCENARIO_BYTES} bytes or is empty")
    _checksum(value["json_sha256"], "scenario JSON checksum")
    chunks = value["chunks"]
    if type(chunks) is not list or not chunks or any(type(chunk) is not str or not chunk for chunk in chunks):
        raise ValueError("Checkpoint compressed scenario chunks must be nonempty strings")
    try:
        compressed = b"".join(base64.b64decode(chunk, validate=True) for chunk in chunks)
    except (ValueError, binascii.Error) as error:
        raise ValueError("Checkpoint compressed scenario contains invalid base64") from error
    ...
    if len(encoded) != size or decoder.unconsumed_tail:
        raise ValueError("Checkpoint expanded scenario size does not match its declaration")
    if not decoder.eof:
        raise ValueError("Checkpoint compressed scenario stream is truncated")
    if decoder.unused_data:
        raise ValueError("Checkpoint compressed scenario has trailing or concatenated streams")
    if hashlib.sha256(encoded).hexdigest() != value["json_sha256"]:
        raise ValueError("Checkpoint scenario JSON checksum does not match expanded contents")
    try:
        return json.loads(encoded.decode("utf-8"), object_pairs_hook=_unique_keys, parse_constant=_reject_constant)
    except (UnicodeError, ValueError, RecursionError) as error:
        raise ValueError(f"Invalid expanded checkpoint scenario JSON: {error}") from error
...
def _state_verification(simulation: CivicSimulation) -> dict[str, Any]:
    ...
    return {"metadata_state_sha256": _digest(state), "trip_geometry_sha256": trip_hashes}
...
def _validate_document(document: Any) -> dict[str, Any]:
    ...
    if type(version) is not int or version not in (1, CHECKPOINT_VERSION):
        raise ValueError("Checkpoint has unsupported checkpoint_version; expected 1 or 2")
    fields = _DOCUMENT_FIELDS | {"verification"} if version == 2 else _DOCUMENT_FIELDS
    if set(document) != fields:
        raise ValueError("Checkpoint object has missing or unsupported fields")
    if document["reconstruction_mode"] != RECONSTRUCTION_MODE:
    ...
    actual_digest = _digest({key: value for key, value in document.items() if key != "sha256"})
    if document["sha256"] != actual_digest:
        raise ValueError("Checkpoint checksum does not match its contents")
    scenario = _decode_scenario(document["scenario"]) if version == 2 else document["scenario"]
    if version == 2:
        proof = document["verification"]
        if type(proof) is not dict or set(proof) != _VERIFICATION_FIELDS:
            raise ValueError("Checkpoint verification has missing or unsupported fields")
        ...
        if document["scenario"]["uncompressed_bytes"] + len(_canonical(document["state"])) > MAX_SCENARIO_BYTES:
            raise ValueError(f"Checkpoint expanded scenario and state exceed {MAX_SCENARIO_BYTES} bytes")
        _validate_tree({"scenario": scenario, "state": document["state"], "verification": proof})
    if type(scenario) is not dict or type(scenario.get("schema_version")) is not int or scenario["schema_version"] != 1:
    ...
    if "sha256" in scenario and _checksum(scenario["sha256"], "embedded scenario sha256") != actual_scenario_hash:
        raise ValueError("Checkpoint embedded scenario checksum does not match its contents")
    ...
    if _finite_number(state["speed"], "state speed", positive=True) != document["speed"]:
...
def prepare_checkpoint_capture(simulation: CivicSimulation) -> PreparedCheckpointCapture:
    ...
    return PreparedCheckpointCapture(encoded, weakref.ref(simulation))
...
def capture_checkpoint(
    ...
    if not isinstance(prepared, PreparedCheckpointCapture) or prepared._simulation() is not simulation:
    ...
    return CheckpointCapture(
        prepared.scenario_json, _canonical(state), simulation.tick,
        simulation.paused, simulation.speed,
    )


def write_captured_checkpoint(capture: CheckpointCapture, path: str | Path) -> Path:
    """Reconstruct and validate a detached capture, then atomically write v2.
    ...
    if type(capture.scenario_json) is not bytes or type(capture.state_json) is not bytes:
        raise ValueError("Checkpoint capture JSON must be immutable bytes")
    if len(capture.scenario_json) + len(capture.state_json) > MAX_SCENARIO_BYTES:
        raise ValueError(f"Checkpoint expanded capture exceeds {MAX_SCENARIO_BYTES} bytes")
    ...
    try:
        scenario = json.loads(
            capture.scenario_json.decode("utf-8"), object_pairs_hook=_unique_keys,
        ...
        reconstructed = simulation.snapshot(include_static=False, include_trip_geometry=False)
    except (UnicodeError, ValueError, TypeError, KeyError, OverflowError, RecursionError) as error:
        raise ValueError(f"Checkpoint capture could not be reconstructed: {error}") from error
    if _canonical(reconstructed) != capture.state_json:
        raise ValueError("Checkpoint capture state does not match deterministic reconstruction")
...
def load_checkpoint(path: str | Path) -> CivicSimulation:
    ...
    try:
        document = json.loads(
            encoded.decode("utf-8-sig"), object_pairs_hook=_unique_keys,
            parse_constant=_reject_constant,
        )
    except (UnicodeError, ValueError, RecursionError) as error:
    ...
    try:
        ...
        reconstructed = simulation.snapshot(include_static=not compact, include_trip_geometry=not compact)
    except (ValueError, TypeError, KeyError, OverflowError) as error:
        raise ValueError(f"Checkpoint scenario could not be reconstructed: {error}") from error
    ...
    expected = dict(document["state"], paused=False, speed=1.0)
    ...
    if compact and _canonical(_state_verification(simulation)) != _canonical(document["verification"]):
        raise ValueError("Checkpoint metadata or trip geometry does not match deterministic reconstruction")
    return simulation
